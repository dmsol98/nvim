return {
  {
    ...
    config = function()
      ...
      end, { desc = "Telescope [F]ind [F]iles in CWD" })
      -- Find ALL files in CWD
      vim.keymap.set("n", "<leader>fF", function()
        builtin.find_files({ hidden = true, no_ignore = true })
      end, { desc = "Telescope [F]ind Hidden [F]iles in CWD" })
      -- Find files in CWD containing string using ripgrep (indlude hidden, no gitignored files, no files in ".git/" folder)
      vim.keymap.set("n", "<leader>fa", function()
        builtin.live_grep({ hidden = true, no_ignore = false, file_ignore_patterns = { "^.git/" } })
      end, { desc = "Telescope [F]ind [A]ll in CWD" })
      ...
      end, { desc = "Telescope [F]ind [T]heme" })
    end,
  },
}
